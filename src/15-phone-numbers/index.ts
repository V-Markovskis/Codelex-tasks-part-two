...
class PhoneNumber {
  ...
  number() {
    // this.input.replace(/[^0-9]/g, '');
    const cleanedInput = this.input.replace(/\D/g, '');
    if(cleanedInput[0] === '1' && cleanedInput.length === 11 && cleanedInput[1] >= '2' && cleanedInput[4] >= '2') {
    ...
    } else if (cleanedInput.length === 10 && cleanedInput[0] >= '2' && cleanedInput[3] >= '2') {
      return cleanedInput;
    }
    return null;
  }
